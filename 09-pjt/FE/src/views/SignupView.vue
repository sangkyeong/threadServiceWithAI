  ...
  <div class="container-fluid bg-dark text-white py-5 min-vh-100 d-flex justify-content-center align-items-start">
    <div class="w-100" style="max-width: 32rem;">
      ...
      <form @submit.prevent="signupHandler">
        ...
        <div v-if="errors.username" class="text-danger small mb-2"><i class="bi bi-exclamation-triangle-fill  me-2"></i>{{ errors.username[0] }}</div>
        
        <input v-model="password1" type="password" placeholder="비밀번호" class="form-control form-control-lg mb-2" :class="{ 'is-invalid': errors.password1 }"/>
        <div v-if="errors.password1" class="text-danger small mb-2"><i class="bi bi-exclamation-triangle-fill  me-2"></i>{{ errors.password1[0] }}</div>
        
        <input v-model="password2" type="password" placeholder="비밀번호 확인" class="form-control form-control-lg mb-2" :class="{ 'is-invalid': errors.password2 }"/>
        <div v-if="errors.password2" class="text-danger small mb-2"><i class="bi bi-exclamation-triangle-fill  me-2"></i>{{ errors.password2[0] }}</div>
        ...
        <select v-model="gender" class="form-select form-control-lg mb-2" :class="{ 'is-invalid': errors.gender }">
